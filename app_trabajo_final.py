...
# CSS para forzar que todo entre en el celular
st.markdown(f"""
    <style>
    ...
    /* Botones rectangulares compactos */
    .stButton>button {{ 
        width: 100%; border-radius: 8px; height: 2.1em; 
        background-color: #e21b2c; color: white; font-weight: bold; border: none;
        font-size: 10px; padding: 0px;
    }}
    
    .reloj-xl {{ color: #e21b2c; font-size: {TAMANO_RELOJ}px !important; font-weight: 900; margin: 0; text-align: center; line-height: 1 !important; }}
    ...
    .texto-info {{
        ...
        align-items: center; height: 45px; line-height: 1.1;
    }}

    .img-milanesa img {{ width: 100% !important; max-height: 200px !important; object-fit: contain; border-radius: 15px; }}
    hr {{ margin: 0.15em 0px !important; opacity: 0.3; }}
    </style>
...
# --- FASE 2: OFERTA RELÁMPAGO (Diseño de tu dibujo) ---
    ...
    st.markdown("<p style='text-align: center; font-size: 12px; margin:0;'>¡Podés agregar un postre a tu compra antes de que salga tu repartidor!</p>", unsafe_allow_html=True)
    ...
    for archivo, nombre, key in postres:
        ...
        with c3:
            st.write("") 
            if st.button("Agregar", key=key):
                st.session_state.fase = 'final'; st.rerun()
        st.divider()

    for t in range(35, -1, -1):
        with reloj_placeholder.container():
            st.markdown(f"<div class='reloj-container'><p style='margin: 0; text-align: center; font-weight: bold; font-size: 10px;'>EL REPARTIDOR SALE EN:</p><p class='reloj-xl'>00:{t:02d}</p></div>", unsafe_allow_html=True)
        time.sleep(1)
